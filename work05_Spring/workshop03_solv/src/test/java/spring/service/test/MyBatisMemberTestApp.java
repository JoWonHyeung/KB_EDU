...
public class MyBatisMemberTestApp {
	...
	public static void main(String[] args) throws Exception{
		...
		//2.
		vo = new MemberVO("321", "123", "아이유2", "방배동");
		row=session.update("MemberMapper.updateMember",vo);
		System.out.println(row+" 명이 수정 성공!!");
		
		session.commit();
		
		//3.
		row=session.delete("MemberMapper.deleteMember","321");
		System.out.println(row+" 명이 삭제 성공!!");
		
		session.commit();
		
		//4.
		vo=session.selectOne("MemberMapper.getMember","888");
